...
{
	LCD_PALETTE_ENTRY_T pal_entry = {.pi = 0}; 
	uint8_t i, *pal_ptr;
	...
	for (i = 0; i < 256 / 2; i++) {
		pal_entry.pe.Bl = (*pal_ptr++) >> 3;	/* blue first */
		pal_entry.pe.Gl = (*pal_ptr++) >> 3;	/* get green */
		pal_entry.pe.Rl = (*pal_ptr++) >> 3;	/* get red */
		pal_ptr++;	/* skip over the unused byte */
		/* do the most significant halfword of the palette */
		pal_entry.pe.Bu = (*pal_ptr++) >> 3;	/* blue first */
		pal_entry.pe.Gu = (*pal_ptr++) >> 3;	/* get green */
		pal_entry.pe.Ru = (*pal_ptr++) >> 3;	/* get red */
		pal_ptr++;	/* skip over the unused byte */

		pLCD->PAL[i] = pal_entry.pi;
	}
}
